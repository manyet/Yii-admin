...
$this->title = Yii::t('app', 'commission_task_benefit');
?>
<?php $this->beginBlock('header') ?>
<a href="<?= Url::toRoute('user/index') ?>" class="back"></a>
<h2 class="f-fs16 fc-fff"><?= Yii::t('app', 'commission_task_benefit') ?></h2>
<?php $this->endBlock() ?>
...
<div class="limit-size">
	<div class="top_div">
		<div class="fc-fff f-fs14 bg-c32545 f-pt15 f-pb15 f-tc limit-size">
			...
			<div class="f-fs22 f-mt10 f-mb5"><?= number_format($user_info['total_task_income'], 2) ?></div>
		</div>
	</div>
...
<li class="f-p10 bordtot-890625 hide">
	<div class="clearfix f-pr list_div">
		<span class="fc-f5ce7f"><?=Yii::t('app','commission_flow_description')?> : </span>
		<span><%=list[i]['flow_description']%></span>
...
function pullupRefresh(isRefresh) {
	$.http.get('<?= \yii\helpers\Url::toRoute('task-commission-list') ?>', {page: page, rows: rows}, {
		before: function() {
