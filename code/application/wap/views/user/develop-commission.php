...
$this->title = Yii::t('app', 'commission_development_reward');
?>
<?php $this->beginBlock('header') ?>
<a href="<?= Url::toRoute('user/index') ?>" class="back"></a>
<h2 class="f-fs16 fc-fff"><?= Yii::t('app', 'commission_development_reward') ?></h2>
<?php $this->endBlock() ?>
...
<div class="limit-size">
	<div class="top_div">
		<div class="fc-fff f-fs14 bg-c32545 f-pt15 f-pb15 f-tc limit-size">
			...
			<div class="f-fs22 f-mt10 f-mb5"><?= number_format($user_info['total_indirect_reward'], 2) ?></div>
		</div>
	</div>
...
<li class="f-p10 bordtot-890625 hide">
	<div class="clearfix f-pr list_div">
        <span class="fc-f5ce7f"><?=Yii::t('app','commission_yesterday_offset_amount')?> :</span>
		<span><%=list[i]['yesterday_commission']%></span>
...
function pullupRefresh(isRefresh) {
	$.http.get('<?= \yii\helpers\Url::toRoute('develop-commission-list') ?>', {page: page, rows: rows}, {
		before: function() {
