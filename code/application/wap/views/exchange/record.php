...
$this->title = Yii::t('app', 'wap_exchange_flowing');
?>
<?php $this->beginBlock('header') ?>
<a href="<?= Url::toRoute('index') ?>" class="back"></a>
<h2 class="f-fs16 fc-fff"><?= Yii::t('app', 'wap_exchange_flowing') ?></h2>
<?php $this->endBlock() ?>
...
<li class="f-p10 bordtot-890625 hide">
	<div class="clearfix">
		<span class="fc-f5ce7f"><?=Yii::t('app','wap_exchange_usage')?></span>
		<span><%=list[i]['casino_name']%></span>
	</div>
	<div class="clearfix f-mt5">
		<span><span class="fc-f5ce7f"><?=Yii::t('app','wap_exchange_buy')?></span> <span class="f-fs16">$<%=moneyFormat(list[i]['amount'])%></span></span>
		<span class="f-fr fc-f5ce7f"><%=dateFormat(list[i]['create_time']*1000,'yyyy-MM-dd hh:mm:ss')%></span>
	</div>
</li>
<%}%>
</script>
