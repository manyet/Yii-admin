...
class Auth
{
    ...
    protected $_config = array(
        ...
        'AUTH_GROUP' => 'tsk_auth_group', //用户组数据表名
        'AUTH_GROUP_ACCESS' => 'tsk_auth_group_access', //用户组明细表
        'AUTH_RULE' => 'tsk_auth_rule', //权限规则表
        'AUTH_USER' => 'tsk_admin'//用户信息表
    );
...
//        $user_groups = M()->table($this->_config['AUTH_GROUP_ACCESS'] . ' a')->where("a.uid='$uid' and g.status='1'")->join($this->_config['AUTH_GROUP'] . " g on a.group_id=g.id")->select();
        ...
        $user_groups = $query->from($this->_config['AUTH_GROUP_ACCESS'] . ' a')->where("a.uid='$uid' and g.status='1'")->leftJoin($this->_config['AUTH_GROUP'], 'g on a.group_id=g.id')->all();
...
//        $rules = M()->table($this->_config['AUTH_RULE'])->where($map)->select();
        $query = new \yii\db\Query();
        $rules = $query->from($this->_config['AUTH_RULE'])->where($map)->all();
        //循环规则，判断结果。
        $authList = array();
        foreach ($rules as $r) {
    ...
    protected function getUserInfo($uid)
    {
        ...
        if (!isset($userinfo[$uid])) {
            $query = new \yii\db\Query();
            $userinfo[$uid] = $query->from($this->_config['AUTH_USER'])->select($uid)->one();
